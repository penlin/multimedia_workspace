...
void BCJR_decoding(const size_t &lu, const int &ind_dec, double* Ly, double* Le1, double* Le2, double* LA){
...
void trellis(){
    ...
    uint8* state_b = MALLOC(uint8,G_M);
    uint8 d_k, a_k, bN, b1;
    uint8** out = new2d<uint8>(2,2);
    uint8** state = new2d<uint8>(2,G_M);
    uint8** nstate = new2d<uint8>(Ns,2);
    ...
    for(int state_i = 0,i = 0, input_bit = 0 ; state_i < Ns ; ++state_i){
        ...
        for(input_bit = 0 ; input_bit <= 1; ++input_bit){
            d_k = input_bit;
            a_k = (G_ptr[0][0]*d_k + InnerProductUint8(G_ptr[0],state_b,1,G_M,0,G_M-1))%2;
            out[d_k][0] = d_k;
            out[d_k][1] = (G_ptr[1][0]*a_k + InnerProductUint8(G_ptr[1],state_b,1,G_M,0,G_M-1))%2;
    ...
    delete2d<uint8>(out);
    delete2d<uint8>(state);
    delete2d<uint8>(nstate);
    delete2d<int8>(nout);
    DELETE(state_b);
}
...
void deinterleave(double** Lu, int** map, const size_t &lm){
#if __INTERLEAVE__
    double* tmp = MALLOC(double,lm);
